...
if(isset($_POST['getTables']))
{
    ...
    { // Löscht den Eintrag und macht somit den Tisch frei.
     ...
     if(isset($_POST['reservierenBtnTisch4Replace']))
    {
    ...
    //Tisch 6 Ende
    //Parkplatz 1
    if(isset($_POST['reservierenBtnpark1Delete']))
    { // Löscht den Eintrag und macht somit den Tisch frei.
      
      $uid = $_SESSION["Park1"][0];
      $sql = "DELETE FROM calenderplanner WHERE uniqueid='$uid'";
      if ($conn->query($sql) === TRUE) {
        $_SESSION["sucessmessagedeleted"] = '<div class="alert alert-success">
            Du hast den Parkplatz 1 wieder frei gemacht.</div>';
            $_SESSION["Park1"] = "FREI";
        header("location:index.php?result=gotrecords");
      } else {
        echo "Error deleting record: " . $conn->error;
     ...
     if(isset($_POST['reservierenBtnpark1Replace']))
    {
      
      $uid = $_SESSION["Park1"][0];
      $name = $_POST["reservierterNameReplacepark"];
      $sql = "UPDATE calenderplanner SET reserviertvon='$name' WHERE uniqueid='$uid'";
      if ($conn->query($sql) === TRUE) {
        $_SESSION["sucessmessagereplaced"] = '<div class="alert alert-success">
            Du hast den Namen der Person an Parkplatz 1 geändert.</div>';
            $_SESSION["Park1"][4] = $name;
    ...
    //Parkplatz 2 Ende
        ...
        //Parkplatz 2
        if(isset($_POST['reservierenBtnpark2Delete']))
        { // Löscht den Eintrag und macht somit den Tisch frei.
          
          $uid = $_SESSION["Park2"][0];
          $sql = "DELETE FROM calenderplanner WHERE uniqueid='$uid'";
          if ($conn->query($sql) === TRUE) {
            $_SESSION["sucessmessagedeleted"] = '<div class="alert alert-success">
                Du hast den Parkplatz 2 wieder frei gemacht.</div>';
                $_SESSION["Park2"] = "FREI";
            header("location:index.php?result=gotrecords");
          } else {
            echo "Error deleting record: " . $conn->error;
         ...
         if(isset($_POST['reservierenBtnpark2Replace']))
        {
          
          $uid = $_SESSION["Park2"][0];
          $name = $_POST["reservierterNameReplacepark2"];
          $sql = "UPDATE calenderplanner SET reserviertvon='$name' WHERE uniqueid='$uid'";
          if ($conn->query($sql) === TRUE) {
            $_SESSION["sucessmessagereplaced"] = '<div class="alert alert-success">
                Du hast den Namen der Person an Parkplatz 2 geändert.</div>';
                $_SESSION["Park2"][4] = $name;
